...
- `ivan.png` (repo root) — sidebar mascot logo; rendered via `st.sidebar.image("ivan.png", width=180)`
...
- **Pure PIL, no numpy**: Browse thumbnails and upload previews are encoded to base64 JPEG and injected via `st.markdown()` HTML `<img>` tags — `st.image()` crashes in Nix for numpy-array inputs. The sidebar mascot uses `st.sidebar.image("ivan.png", width=180)` which works fine because it passes a file path (PIL reads it directly, no numpy array conversion).
...
- Sidebar: white background, Ivan mascot via `st.sidebar.image()` (180px wide), crimson title, gray divider above nav
...
- **Never use `st.image()` with a numpy array** — crashes in Nix (missing `libstdc++.so.6`); use base64 HTML `<img>` for thumbnails/previews. File-path inputs to `st.sidebar.image()` are safe (PIL reads directly, no numpy array).
...
- **Don't base64-encode large images into `st.sidebar.markdown()`** — Streamlit's markdown renderer silently drops `data:` URIs over ~1 MB; use `st.sidebar.image()` with a file path instead
- `ivan.png` (lowercase, no space) — renamed from `Ivan .png`; lives at repo root; pushed to GitHub
